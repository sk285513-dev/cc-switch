...
def get_last_log_lines(n: int = 10) -> list[str]:
    try:
        return LOG_PATH.read_text(encoding="utf-8-sig", errors="replace").splitlines()[-n:]
    except Exception:
        return []
...
def get_workflow_pid() -> list[int]:
    ...
    try:
        out = subprocess.check_output(
            ...
            shell=True, text=True, errors='ignore', timeout=5, creationflags=0x08000000
        )
        pids = []
...
def autofix(diag: dict) -> list[str]:
    ...
    if quota_state.exists():
        try:
            qs = json.loads(quota_state.read_text(encoding="utf-8-sig"))
            # 只清除短期限速（保留 limit:0 的永久封鎖）
            if qs.get("exhausted_keys"):
                qs["exhausted_keys"] = []
                quota_state.write_text(json.dumps(qs, indent=2), encoding="utf-8-sig")
    ...
    if not diag["workflow_alive"]:
        try:
            script = r"C:\LocalAI_Workstation\scripts_v6\run_workflow.py"
            subprocess.Popen(
...
def write_alert(diag: dict, stage: str, actions: list[str], reason: str):
    ...
    ALERT_PATH.write_text(json.dumps(alert, ensure_ascii=False, indent=2), encoding="utf-8-sig")
