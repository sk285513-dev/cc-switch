...
from auto_ingest_bot import scan_drives, get_video_duration_and_size, load_ingested_history, get_processed_stems
...
def generate_failed_srt_list():
    ...
    failed_files.sort(key=lambda x: (get_exam_score(x['course']), x['course'], x['video_name']))
