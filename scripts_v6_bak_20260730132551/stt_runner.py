...
# [EXPERT FIX] 升級為 Manager Queue，確保跨進程 (Multi-processing) 安全
manager = multiprocessing.Manager()
manifest_update_queue = manager.Queue()
...
def manifest_writer_thread(chunks_manifest_path, manifest_update_queue):
    """背景單一寫入者：定期將 Queue 中的狀態批次寫入硬碟"""
    logging.info("[ManifestWriter] Background writer started.")
    while True:
        updates = []
        try:
            # 阻塞等待，最多等 2 秒，達成批次寫入 (Batching)
            ...
            if os.path.exists(chunks_manifest_path):
                ...
                for c in current_chunks:
                    for u in updates:
                        if c.get("filename") == u["filename"]:
                            c["status"] = u["status"]
                            if "started_at" in u:
                                c["started_at"] = u["started_at"]
                            updated = True
                
                if updated:
                    atomic_json_dump(current_chunks, chunks_manifest_path)
...
def process_single_chunk(
    ...
    while chunk["retry_count"] < retry_limit:
        try:
            # 【修復因果律】：在正式呼叫 STT 引擎「之前」，發送 processing 狀態與時戳
            manifest_update_queue.put({"filename": chunk_filename, "status": "processing", "started_at": time.time()})
            ...
            if stt_engine in ["gemini", "vertexai"]:
                ...
                # 專家修復：將 Distill 任務放入有數量限制的 Queue 中，並傳遞所需參數，避免新開進程
            ...
            # 專家修復：將完成狀態放入 Queue，由背景執行緒非同步寫入，徹底解除 I/O 阻塞
            manifest_update_queue.put({"filename": chunk_filename, "status": "completed"})
            return True
            ...
            if stt_engine == "gemini" and is_429:
                ...
                time.sleep(15 * (chunk["retry_count"] + 1))
                continue
