...
def main():
    ...
    sync_res = subprocess.run([sys.executable, sync_script], capture_output=False, creationflags=0x08000000)
    ...
    if os.path.exists(mcp_exe):
        ...
        res = subprocess.run(cmd, capture_output=True, text=True, encoding="utf-8-sig", creationflags=0x08000000)
        if res.returncode == 0:
