...
from model_router import get_router  # Model Router Agent — 全權負責模型切換
...
# Goal
你的任務是接收我提供的音檔，將其轉錄為一份專業、流暢、術語精準且符合閱讀邏輯的「完整逐字稿檔案」。

# Strict Rules (核心修訂原則)
1. **格式與時間軸絕對恆定**：請嚴格加上時間戳記格式（如 `[00:00]`）。時間軸數字絕對不能有任何變動，亦不能合併或刪除。
2. **修正語音辨識錯誤（空耳）**：語音辨識系統常因音近而誤判專業術語。請根據「領域背景」與「前後文邏輯」，將語意不通的聲音修正為正確的專業詞彙、法條、或學者姓名。
3. **口語贅詞流暢化**：在不影響時間軸對齊的前提下，適度刪除或精簡語音中無意義的贅字（如：「好」、「那」、「這個這個」、「然後」、「對」），提高字幕的可讀性。
4. **修復系統死當/鬼打牆區段**：若語音中出現重複字句，請「不要照抄」。你必須展現高超的邏輯推理能力，依據前後文脈絡，將受損區段重構為通順、合理的專業內容，並補回對應的時間軸內。
5. **絕對禁止截斷與摘要**：我需要的是「完整的逐字稿精校」，你必須將所有的音訊從頭到尾完整處理並輸出。絕對不能在中途寫出「（以下省略）」或只提供摘要。
