...
KPI_B2_MIN_CHUNKS_PER_5MIN   = 3      # 每 5min 至少 3 個新 chunk
...
def run_kpi_check():
    # 《附註》必須用台灣時間 UTC+8，不能用 datetime.now()（伺服器系 UTC）
    ...
    if is_stt_phase:
        ...
        log(f"  {'✅' if ok else '❌'} KPI-B2: +{realtime_chunk_count} chunks/5min (需 ≥ {KPI_B2_MIN_CHUNKS_PER_5MIN}) [即時掃描]")
        # 同時顯示快照差值供參考
