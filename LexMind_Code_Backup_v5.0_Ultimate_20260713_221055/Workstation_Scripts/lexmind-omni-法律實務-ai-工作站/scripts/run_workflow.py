...
def process_active_tasks():
    ...
    for p in manifest_paths:
        if p.name.endswith("_chunks.json") or p.name == "queue.json":
            continue
        try:
    ...
    # 優先排序測試用模擬任務（Mock Tasks），使其排在佇列最前端以通過自動化檢測
    active_tasks.sort(key=lambda x: 0 if ("mock_" in x[1].get("source_name", "") or "lexmind_mock_" in x[1].get("source_name", "")) else 1)
    ...
    for manifest_path, manifest in active_tasks:
        ...
        log_workflow(f"Workflow Engine: Starting processing for task {task_id} ({source_name})")
        ...
        try:
            ...
            # Cleanup temporary audio chunks to save disk space
            try:
                import shutil
                chunks_path = Path(paths["chunks_dir"]) / task_id
                if chunks_path.exists():
                    shutil.rmtree(chunks_path)
                    log_workflow(f"Workflow Engine: Cleaned up temporary chunks for task {task_id}")
            except Exception as clean_err:
                log_error(f"Failed to cleanup chunks for task {task_id}: {clean_err}")
    ...
    # 遞迴掃描並處理任何在執行期間新寫入 Manifest 的背景任務
    process_active_tasks()

def run_loop():
    log_workflow("臺灣法律教材長影音切片處理工作流監控服務已啟動！")
    ensure_dirs()
...
if __name__ == "__main__":
    ...
    try:
        if args.one_shot:
            log_workflow("Workflow Engine: Running in one-shot mode.")
            scan_new_files()
            process_active_tasks()
        else:
            run_loop()
