...
def init_system_resources():
    try:
        ...
        if sys.platform == "win32" and ctypes.windll.shell32.IsUserAnAdmin() != 0:
            script_path = "C:/LocalAI_Workstation/scripts/law_scraper_cli.py"
...
from scripts.visual_analyzer import VisualLegalAnalyzer
...
with tab_cases:
    ...
    from scripts.case_manager import load_cases, save_cases
    cases = load_cases()
    ...
    if submit_exam:
        with st.spinner("📝 AI正在嘗試作答，並由閱卷教授對照高分解答進行檢討中..."):
            try:
                ...
                res_ans = requests.post(f'{os.environ.get("OLLAMA_HOST", "http://localhost:11434")}/api/chat', json=payload_ans, timeout=90)
                if res_ans.status_code == 200:
                    ai_answer = res_ans.json().get('message', {}).get('content', 'API 回應格式異常')
                else:
                    ai_answer = f"錯誤: 伺服器響應代碼 {res_ans.status_code}"
                ...
                res_crit = requests.post(f'{os.environ.get("OLLAMA_HOST", "http://localhost:11434")}/api/chat', json=payload_crit, timeout=90)
                if res_crit.status_code == 200:
                    critique = res_crit.json().get('message', {}).get('content', 'API 回應格式異常')
                else:
                    critique = f"錯誤: 伺服器響應代碼 {res_crit.status_code}"
                
                # 固化記憶
                lesson_doc = f"【教授精算檢討盲點】：\n{critique}\n\n【題目】：\n{question_text}"
                lesson_embedding = st.session_state.agent_instance._get_embedding(lesson_doc)
                if lesson_embedding:
                    st.session_state.agent_instance.intel_coll.add(
                        ids=[f"exam_lesson_{os.urandom(3).hex()}"],
                        documents=[lesson_doc],
                        embeddings=[lesson_embedding],
                        metadatas=[{"source": "國家司法考試自我檢討", "level": 1}]
                    )
