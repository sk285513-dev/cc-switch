...
def render_rich(tasks, fmt_events, export=False):
    ...
    stt_act  = sum(1 for t in tasks if "STT" in t["stage"])
    ...
    for t, fmt_ts in sorted_done:
        ...
        dur = f"~{t['n_chunks']*12}min" if t["n_chunks"] else "—"
        tbl.add_row(ts_str2, t["name"][:38], str(t["n_chunks"]) if t["n_chunks"] else "—", dur)
    ...
    if active:
        tbl2 = Table(title=f"🔄 進行中任務（{len(active)} 堂）",
        ...
        def sort_key(t):
            for k,v in stage_order.items():
                if t["stage"].startswith(k): return v
            return 9
