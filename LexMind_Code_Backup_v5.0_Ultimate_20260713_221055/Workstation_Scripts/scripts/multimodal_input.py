...
def generate_content_with_file(file_uri, mime_type, prompt, api_key, model_name="gemini-3.5-flash"):
    """
    呼叫 Gemini generateContent。
    模型切換全權由 ModelRouter Agent 負責（見 model_router.py）。
    """
    if _HAS_ROUTER:
        # 通過 Router 自動切換
        return _router_rest_generate(file_uri, mime_type, prompt, api_key)
    # Fallback: 直接呼叫（Router 未安裝時）
...
class MultimodalLegalInput:
    ...
    def transcribe_audio(self, file_path, status_callback=None):
        ...
        try:
            ...
            # 3. 呼叫 Gemini 雲端
            # 《修復》統一由 ModelRouter Agent 决定模型，不再讀 config
            try:
                from scripts.model_router import get_router as _get_mr
                model_name = _get_mr().acquire()
            except Exception:
