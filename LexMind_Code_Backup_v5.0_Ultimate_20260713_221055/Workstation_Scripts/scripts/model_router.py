...
【使用方法 - SDK 呼叫（stt_runner.py）】
  from scripts.model_router import get_router
  router = get_router()
...
【使用方法 - REST 呼叫（multimodal_input.py / visual_analyzer.py）】
  from scripts.model_router import rest_generate
...
# ─────────────────────────────────────────────────────────────────
# 【品質優先序》一律依【法律繁體中文轉譯精確度】降序排列
# gemini-3.5-flash:   《前沖級》 high-level reasoning，同音字校正、法條辨識最強 ← STT 首選
# gemini-3-flash-preview: 《中沖級》 備援一，適合大量總合/merge
# gemini-3.1-flash-lite:  《輕量級》 最小模型，只適合簡單下載/upload 操作或最後手段備援
MODELS_BY_PRIORITY: list[str] = [
    "gemini-3.5-flash",           # ★★★ 首選：前沖級推理，法律逐字稿最精確
    "gemini-3-flash-preview",     # ★★  備援一：中沖級，完整度與透毀率安定
    "gemini-flash-latest",        # ★★  3.5 別名，與 3.5 共用限速
    "gemini-3.1-flash-lite",      # ★   輕量備援：由流動容領高須由前三個處理
    "gemini-flash-lite-latest",   # ★   3.1-lite 別名
]

# 永久禁止清單（limit:0 或確認無配額）
BLACKLISTED_MODELS: set[str] = {
    "gemini-2.5-flash",       # 免費帳號 404
    ...
    def acquire(self) -> str:
        ...
        with self._lock:
            ...
            if not available:
                ...
                logger.warning(
                    f"[ModelRouter] ⚠️ 所有模型冷卻中！最快恢復: {best}"
                    f"（{remaining:.0f}s 後）"
                )
                return best
