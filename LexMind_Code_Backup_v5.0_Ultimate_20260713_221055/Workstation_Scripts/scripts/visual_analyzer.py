...
try:
    from scripts.model_router import rest_generate as _router_rest_generate
    _HAS_ROUTER = True
except ImportError:
    _HAS_ROUTER = False
...
class VisualLegalAnalyzer:
    ...
    def _query_gemini_multimodal(self, image_path, prompt, api_key):
        try:
            ...
            url_status = f"https://generativelanguage.googleapis.com/v1beta/{file_name_api}?key={api_key}"
            for _ in range(30):
            ...
            # 透過 ModelRouter 呼叫 Gemini（自動切換模型）
            if _HAS_ROUTER:
                try:
                    content = _router_rest_generate(
                        file_uri, mime_type, prompt, api_key
                    )
                except Exception as e:
                    print(f"[Gemini OCR] ModelRouter REST 失敗: {e}")
                    content = ""
            else:
                # Fallback: 直接呼叫
                ...
                try:
                    cfg = load_config()
                    model_name = cfg.get("api", {}).get("gemini_model_low_cost", "gemini-3.5-flash")
                except:
                    pass
                url_gen = (f"https://generativelanguage.googleapis.com"
                ...
                if res_gen.status_code == 200:
                    try:
                        ...
                        print(f"[Gemini OCR] Parse failed: {parse_e}")
                else:
                    print(f"[Gemini OCR] HTTP {res_gen.status_code}: {res_gen.text[:200]}")
                    if res_gen.status_code == 429:
                        self.gemini_multimodal_disabled = True
            
            # Cleanup
            try:
                requests.delete(f"https://generativelanguage.googleapis.com/v1beta/{file_name_api}?key={api_key}")
            except:
                pass
