...
class QuotaManager:
    def __init__(self, keys_path: str = "config/keys.yaml", policy_path: str = "config/quota_policy.yaml", state_path: str = "config/quota_state.db"):
        self.keys_path = Path(keys_path)
        self.policy_path = Path(policy_path)
        self.state_path = Path(state_path)
        self._mutex = win32event.CreateMutex(None, False, "LexMind_Quota_Mutex")
        self._init_db()
        self._load_keys()
        self._load_policy()
        self._load_state()
    ...
    def _ipc_lock(self):
        res = win32event.WaitForSingleObject(self._mutex, 15000)
        if res == win32event.WAIT_TIMEOUT:
            raise TimeoutError("IPC Mutex Timeout: Unable to acquire QuotaManager lock.")
        try:
            yield
        finally:
    ...
    def _init_db(self):
        ...
        conn = sqlite3.connect(self.state_path, timeout=5.0)
        conn.execute("PRAGMA journal_mode=WAL;")
        conn.execute("CREATE TABLE IF NOT EXISTS exhausted_keys (key TEXT PRIMARY KEY)")
        conn.commit()
        conn.close()
    ...
    def _load_state(self):
        ...
        if self.state_path.exists():
            try:
                conn = sqlite3.connect(self.state_path, timeout=5.0)
                cursor = conn.execute("SELECT key FROM exhausted_keys")
                exhausted_keys = [row[0] for row in cursor.fetchall()]
                conn.close()
            except Exception as e:
    ...
    def _save_state(self):
        """Must be called while holding self._ipc_lock(), or from within a locked method."""
        try:
            os.makedirs(self.state_path.parent, exist_ok=True)
            conn = sqlite3.connect(self.state_path, timeout=5.0)
            conn.execute("PRAGMA journal_mode=WAL;")
            conn.execute("BEGIN IMMEDIATE")
            conn.execute("DELETE FROM exhausted_keys")
            exhausted = [(k,) for k, info in self.key_status.items() if info["exhausted"]]
            if exhausted:
                conn.executemany("INSERT INTO exhausted_keys (key) VALUES (?)", exhausted)
            conn.commit()
            conn.close()
        except Exception as e:
