...
from auto_ingest_bot import scan_drives, SUPPORTED_EXTS, is_file_content_legal, get_video_duration_and_size, is_file_already_ingested, load_ingested_history, get_processed_stems
...
def generate_perfect_list():
    ...
    perfect_files.sort(key=lambda x: (get_legal_priority_score(x['course']), x['course'], x['file_name']))
