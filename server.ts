  ...
  return new Promise<void>((resolve) => {
    ...
    client.once("error", () => {
      ...
      const childEnv = {
        ...
        OLLAMA_KEEP_ALIVE: "15m", OLLAMA_MODELS: "C:\\Users\\temp"
      };
      try {
...
async function ensureBothOllamas(): Promise<void> {
  const model = process.env.OLLAMA_MODEL || "deepseek-r1:32b";
...
async function startServer() {
  ...
  app.post("/api/generate-video", async (req, res) => {
    try {
      ...
      console.log(`[T2V ENGINE] Received prompt: ${prompt} | Style: ${style}`);
      // Simulate video generation delay (5 seconds)
      await new Promise(resolve => setTimeout(resolve, 5000));
      // Mock video response - placeholder video
      const mockVideoUrl = "https://test-videos.co.uk/vids/bigbuckbunny/mp4/h264/360/Big_Buck_Bunny_360_10s_1MB.mp4";
      res.json({ status: "success", videoUrl: mockVideoUrl });
    } catch (e: any) {
    ...
    if (!isMock) {
      try {
        ...
        if (process.env.USE_OLLAMA === "true") {
          const ollamaHost = process.env.OLLAMA_HOST || "http://127.0.0.1:11436";
          const ollamaModel = process.env.OLLAMA_MODEL || "deepseek-r1:32b";
          console.log(`[LAWYER CHAT] Forwarding final inference to local model "${ollamaModel}" at "${ollamaHost}"`);
          // ★ 修正：改用 /api/chat 並分離 system/user 角色，避免 raw prompt 造成答非所問
          let additionalMemory = "";
          try {
            const memPath = "C:/LocalAI_Workstation/model_memory.txt";
            if (require("fs").existsSync(memPath)) {
              additionalMemory = "\n\n【重要實務見解修正與強制記憶 (Priority Rule)】\n" + require("fs").readFileSync(memPath, "utf-8");
            }
          } catch (e) {}

          const systemMsg = `你是一個精通臺灣法律實務、擁有「老法官與老律師靈魂」的頂級 AI 法律特助。
請根據【RWS 檢索法規智識庫】提供的參考判例條文，給予嚴謹、具備高度實務法感、條理分明的法律解答。
- 答題採用「三段論法」（法規依據→具體 facts 涵攝對位→下一步救濟行動）。
- 使用正確台灣法律優雅用語，絕對不得出現大陸法律詞彙，繁體中文。
- 如涉及消滅時效或追訴權且有過期風險，必須在第一段最開頭嚴厲警示。
法律角色視角：當前以『${context_role === "lawyer" ? "申訴人/律師時效防禦" : context_role === "judge" ? "法官客觀審判心證" : "檢察官刑事求處"}』心證出發。${additionalMemory}`;
          const userMsg = `【RWS 檢索法規與智識庫（已優先展示黃金條文與特殊時效盾牌）】：
${context_str}

【當事人事實與提問】：
${condensedQuery}`;
          const startTime = Date.now();
          const oRes = await fetch(`${ollamaHost}/api/chat`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              model: ollamaModel,
          ...
          if (oRes.ok) {
            ...
            // Log inference stats to CSV
            try {
              const logsDir = "C:/LocalAI_Workstation/logs";
              if (!fs.existsSync(logsDir)) {
                fs.mkdirSync(logsDir, { recursive: true });
              }
              const statsFile = path.join(logsDir, "inference_stats.csv");
              if (!fs.existsSync(statsFile)) {
                fs.writeFileSync(statsFile, "timestamp,latency_ms\n", "utf-8");
              }
              const timestamp = new Date().toISOString();
              fs.appendFileSync(statsFile, `${timestamp},${latencyMs}\n`, "utf-8");
            } catch (logErr) {
              console.error("Failed to log inference stats:", logErr);
            }
            
            // ★ Bug #4 修正：deepseek-r1:32b 是 Thinking Model
            // 正常輸出：content = 最終答案，thinking = 推論過程
            // 若 content 為空（代表 token 被截斷在思考中），fallback 讀 thinking
            const rawContent = data.message?.content || "";
            const rawThinking = data.message?.thinking || data.response || "";
            reply = rawContent.trim() || rawThinking.trim() || "";
          } else {
            const errText = await oRes.text();
            throw new Error(`Local model generation failed: HTTP ${oRes.status} ${errText}`);
          }
        } else {
...
請立刻調用本工作站「時效精算外掛」確立保命符終期，並參照本站範本一鍵自動編起訴書狀。`;
  ...
  app.get("/api/inference-stats", (req, res) => {
    try {
      const statsFile = "C:/LocalAI_Workstation/logs/inference_stats.csv";
      if (!fs.existsSync(statsFile)) {
        return res.json({ avgWaitTimeSeconds: 45 }); // Default 45s
      }
      ...
      if (lines.length === 0) {
        return res.json({ avgWaitTimeSeconds: 45 });
      }
      
      // Rolling average of the last 20 requests
      ...
      const avgMs = totalLatencyMs / validCount;
      const avgSecs = Math.round(avgMs / 1000);
      // Minimum wait time threshold 10s to avoid UI glitches
      return res.json({ avgWaitTimeSeconds: Math.max(10, avgSecs) });
    } catch (err) {
      console.error("Failed to calculate inference stats:", err);
      return res.json({ avgWaitTimeSeconds: 45 });
    }
  });

  app.post("/api/eval-local", async (req, res) => {
    try {
      ...
      const ollamaHost = process.env.OLLAMA_HOST ? process.env.OLLAMA_HOST.replace("11434", "11436") : "http://127.0.0.1:11436";
      const ollamaModel = process.env.OLLAMA_MODEL || "deepseek-r1:32b";

      // 1. Get Local Model Answer
      let localAnswer = "";
      try {
        const oRes = await fetch(`${ollamaHost}/api/chat`, {
          method: "POST",
          ...
          body: JSON.stringify({
            model: ollamaModel,
            messages: [
              { role: "system", content: "你是一位精通台灣法律的律師。" },
        ...
        if (oRes.ok) {
          const data = await oRes.json();
          localAnswer = data.message?.content || data.message?.thinking || data.response || "";
        } else {
          return res.status(500).json({ error: "Local model failed" });
        }
      } catch (e) {
        return res.status(500).json({ error: "Local model error", details: e.message });
      }

      // 2. Get Cloud Model Answer (Baseline)
      const cloudResp = await ai.models.generateContent({
        model: "gemini-3.5-flash",
        contents: `你是一位資深台灣法官。請簡答以下問題，必須引用最新實務決議或法條：\n${question}`,
        config: { temperature: 0.1 }
      });
      const cloudAnswer = cloudResp.text || "";

      // 3. Ask Cloud to Evaluate the Local Answer
      const evalPrompt = `你現在是台灣法學專家兼 AI 訓練師。
以下有一個【法律問題】，以及兩個 AI 模型給出的回答。
【問題】：${question}
【本地模型回答】：${localAnswer}
【雲端模型回答（標準答案）】：${cloudAnswer}

請判斷【本地模型回答】是否在法規引用、實務見解或邏輯上存在嚴重錯誤、遺漏或過時。
如果「完全正確無誤」，請回覆純字串 "CORRECT"。
如果有錯或有過時遺漏，請產出一段「簡短的修正指引（Knowledge Correction）」，語氣必須是直接的法律規則，例如：「注意：根據106年第3次民事庭會議決議，借名登記出名人之處分屬有權處分，不論第三人善惡意皆取得所有權。」
請只輸出 "CORRECT" 或那一段指引，不要有多餘廢話。`;
      ...
      const evaluation = (evalResp.text || "").trim();

      // 4. Update Memory if needed
      let memoryUpdated = false;
      if (evaluation && evaluation !== "CORRECT" && !evaluation.includes("CORRECT")) {
        try {
          const fs = require("fs");
          const memPath = "C:/LocalAI_Workstation/model_memory.txt";
          let memories = [];
          if (fs.existsSync(memPath)) {
            memories = fs.readFileSync(memPath, "utf-8").split("\n---\n").filter(m => m.trim().length > 0);
          }
          memories.push(evaluation);
          fs.writeFileSync(memPath, memories.join("\n---\n"), "utf-8");
          memoryUpdated = true;
        } catch (memErr) {
      ...
      console.error(e);
      res.status(500).json({ error: "Eval logic failed", details: e.message });
    }
  });
...
3. 訴訟下一步指引：請妥適保存事證，以便於庭期日前向法院呈遞「民事/刑事答辯狀」。`;
  ...
  app.get("/api/scan-local-media", (req, res) => {
    ...
    const isMyComputer = lowercaseDir === "my-computer" || lowercaseDir === "本機" || lowercaseDir === "this pc" || lowercaseDir === "thispc";
    const allowedExts = ["mp4", "mp3", "wav", "txt", "pdf"];
    try {
      let allFiles = [];
      if (isMyComputer) {
        const letters = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
  ...
  app.post("/api/parse-local-file", async (req, res) => {
    ...
    try {
      ...
      if (!res.headersSent && !timedOut) {
        res.status(500).json({ error: `GPU 資源調度失敗: ${e.message || String(e)}` });
      }
      return;
    }
    try {
  ...
  app.post("/api/parse-image", async (req, res) => {
    try {
      ...
      const prompt = "請對這張圖片進行多模態高精準度 OCR ＆ 案件文書圖像分析，並將黑板上的圖畫、複雜法律關係圖表、或文件內容翻譯成結構化的 Markdown 文字筆記。請直接輸出識別與分析的結果，不要有額外的問候語。";
      
      // Try Gemini Flash first if available
      if (process.env.GEMINI_API_KEY) {
        ...
        try {
          ...
          return res.json({ status: "success", content: `【多模態高精準度 OCR ＆ 案件文書圖像分析】\n檔案名稱: ${fileName}\n===============================================\n${response.text()}` });
        } catch (e) {
      ...
      // Fallback to Ollama if USE_OLLAMA is true
      if (process.env.USE_OLLAMA === "true") {
        ...
        try {
          ...
          console.error("Ollama Vision OCR error:", e);
        }
      }
      
      return res.json({ status: "success", content: `【多模態高精準度 OCR ＆ 案件文書圖像分析】\n檔案名稱: ${fileName}\n===============================================\n[無法進行 OCR 識別：無可用的視覺模型或 API 發生錯誤。]` });
      
    } catch (e) {
      console.error("/api/parse-image error:", e);
...
`;
  ...
  app.get("/api/system-status", async (req, res) => {
    ...
    try {
      ...
      if (oRes.ok) {
        ...
        if (data.models && Array.isArray(data.models)) {
          ...
          if (!availableModels.includes(ollamaModel)) {
            availableModels.unshift(ollamaModel);
          }
        }
      } else {
        ollamaStatus = "error";
      ...
      console.error(`[OLLAMA API ERROR] Failed to fetch tags from ${ollamaHost}:`, err.message || err);
    ...
    res.json({
      ollama: {
        ...
        cloudModels: ["gemini-2.5-flash", "gemini-2.5-pro", "gemini-3.5-flash"]
      },
      scraper: {
