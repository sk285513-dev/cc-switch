...
├── scripts/
│ ├── agent_core_pro.py # LocalLegalAgent 核心 (Agent ①③④⑦⑧⑨)
│ ├── auto_ingest_bot.py # 全自動教材吸收機器人 (Agent ②⑥)
│ ├── web_scraper.py # 法律網頁爬蟲 (Agent ③)
│ ├── exam_agent.py # 司法考試培訓 Agent (Agent ⑤)
│ ├── exam_trainer.py # 國考雙 Agent 閉環檢討核心 (訓練本體)
│ ├── batch_exam_trainer.py # 國考批次訓練調度器
│ ├── run_workflow.py # 長影音批次轉譯工作流核心調度器
│ ├── preprocess_media.py # 影音規格轉換與前處理
│ ├── chunk_planner.py # 智慧斷句靜音切片排程器
│ ├── stt_runner.py # ASR 語音轉譯 (429 退避與金鑰池輪替)
│ ├── merge_transcript.py # 轉譯文本合併與段落校正
│ ├── markdown_formatter.py # Markdown 講義與字幕格式化輸出 (自動備份回原目錄)
│ ├── file_watcher.py # 本地目錄自動監視器
...
為了在處理大批量長影音教材（如 30 堂大型法律教材）時確保零當機與零配額浪費，教材吸收管線整合了以下三大健全設計：

1. **雲端 STT 429 頻率限制自適應退避**：
   * 在使用 Gemini Files API 進行語音聽寫時，若遇到 `429 RESOURCE_EXHAUSTED` 頻率超限錯誤，系統會自動使用正則表達式解析錯誤訊息中的等待秒數 `Please retry in (\d+\.?\d*)s`。
   * 系統會原地睡眠等待指定的秒數（外加 2 秒安全期）後重新嘗試，避免在頻率超限時盲目切換金鑰導致金鑰被全部鎖定。

2. **金鑰池 Round-Robin 輪替與冷卻機制**：
   * 系統在 `config.yaml`、`.env` 與環境變數中支援配置多組 Gemini API Key（金鑰池）。
   * 當同一個分片在同一個 API Key 下連續重試失敗 **3 次**後，系統將判定該 Key 的每日配額已耗盡，呼叫 `mark_key_exhausted()` 將其狀態標記為 `exhausted`，並記錄耗盡時間戳，進入 **10 分鐘冷卻期**。
   * 系統自動輪替至金鑰池中的下一個可用 Key。若池內所有金鑰皆已耗盡，進程將大睡眠 60 秒後，重設金鑰狀態並重新循環嘗試，防止程式崩潰中斷。

3. **分片進度即時 Checkpoint 存檔**：
   * 音訊切片（每段約 12-20 分鐘）在上傳轉譯時，系統將原本「整堂課完成才存檔」的邏輯優化為**「每成功轉譯一個分片，立刻更新並存檔 chunks_manifest.json」**。
   * 當遭遇斷電、手動熱切斷或版本更新中斷時，重啟工作流將直接讀取 Manifest，自動跳過已完成的分片，絕不重複呼叫 API 消耗額度。

4. **格式化輸出與影片原目錄備份落地**：
   * 在 `markdown_formatter.py` 合併去冗校正完成並將 5 大落地檔案（`.md`, `.srt`, `.vtt`, `.txt`, `_index.json`）寫入 `A:\processed_md\` 的同時，系統會讀取 Manifest 中的 `source_path` 參數取得原始外接硬碟/隨身碟路徑。
   * 自動呼叫 `shutil.copy` 將這 5 個產出檔案備份一份存回影片原始目錄中（例如 `J:\土地登記\ch1\`），方便使用者隨時就近查閱。
