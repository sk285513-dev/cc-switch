...
def parse_transcript_into_timed_segments(text, chunk_start_time):
    ...
    segments = []
    if not matches:
        # 若找不到時間戳，依照字串長度與總時間 (預設 600 秒) 平均分割區塊
        seg_length = max(15, len(text) // 40)
        for i in range(0, len(text), seg_length):
            seg_text = text[i:i+seg_length].strip()
            if seg_text:
                segments.append({
                    "start": chunk_start_time + (i / max(1, len(text))) * 600.0,
                    "end": chunk_start_time + min(600.0, ((i + seg_length) / max(1, len(text))) * 600.0),
                    "text": seg_text
                })
        if not segments:
            segments.append({"start": chunk_start_time, "end": chunk_start_time + 600.0, "text": text})
        return segments
...
def save_distillation_pair(task_id, source_name, spec_subject, full_text, cleaned_text, chunks_data):
    ...
    try:
        ...
        def safe_append(path, data):
            for _ in range(50):
                try:
                    ...
                    time.sleep(0.1)
            print(f"[Markdown Formatter] Failed to append to {path} after retries.")
        ...
        safe_append(out_path, json.dumps(pair, ensure_ascii=False) + "\n")
        pairs_written += 1
        ...
        for idx, chunk in enumerate(chunk_list):
            ...
            safe_append(out_path, json.dumps(pair_chunk, ensure_ascii=False) + "\n")
            pairs_written += 1
...
"""
    ...
    try:
        ...
        from scripts.security_utils import clean_env
        ...
        if os.path.exists(target_script):
            ...
            try:
                subprocess.run([sys.executable, target_script], cwd=os.path.dirname(target_script), env=clean_env(), timeout=600, check=True)
            except subprocess.TimeoutExpired:
                log_error(f"Markdown Formatter: Qdrant ingest script timed out after 600s!")
            except subprocess.CalledProcessError as e:
                log_error(f"Markdown Formatter: Qdrant ingest script failed with return code {e.returncode}")
        else:
