...
def call_gemini_api(prompt, model_name, qm, current_key_ref):
    ...
    for current_model in models_to_try:
        ...
        while attempt < 3:
            ...
            try:
                ...
                if (isinstance(e, httpx.HTTPStatusError) and e.response.status_code == 429) or ("429" in str(e) or "RESOURCE_EXHAUSTED" in str(e) or "quota" in str(e).lower()):
                    err_str = str(e)
                    wait_sec = None
                    ...
                    print(f"API Key {api_key[:8]}...{api_key[-4:]} 異常 (可能是額度限制): {e}。正在由 QuotaManager 處理...")
                    res = qm.handle_error(e, api_key, consecutive_429_count=1)
                    if res["sleep_time"] > 0:
                        time.sleep(res["sleep_time"])
                    if res["new_key"]:
                        current_key_ref[0] = res["new_key"]
...
def merge_workflow(task_id):
        ...
        if not merged_text:
            ...
            if match.size >= 12:
                ...
                if api_key:
                    ...
                    try:
                        refined_gap = call_gemini_api(PROMPT_GAP_REFINE.format(text=raw_gap), model_name, qm, current_key_ref)
    ...
    for idx, chunk in enumerate(chunks):
        ...
        if api_key:
            ...
            try:
                block_summary = call_gemini_api(PROMPT_SUMMARY.format(text=chunk_content), model_name, qm, current_key_ref)
