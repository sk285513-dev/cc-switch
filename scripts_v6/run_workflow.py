...
def _bootstrap_gcp_project() -> None:
    ...
    if config_path.exists():
        try:
            ...
            print(f"[BOOTSTRAP] WARNING: 讀取 config.yaml 失敗: {_e}", file=sys.stderr)
...
def atomic_json_dump(data, filepath):
    tmp_path = str(filepath) + f".{os.getpid()}.{threading.get_ident()}.tmp"
    ...
    if not chunking_only and exclusive_key is None and qm is not None:
        try:
            ...
            if exclusive_key == "__VERTEX_AUTH__":
                ...
                exclusive_key = None   # 傳 None 給 _run_task_steps，讓它走 vertexai=True
            else:
...
def process_active_tasks(chunking_only=False):
    ...
    if stt_engine == "vertexai" and merge_engine == "vertexai":
        log_workflow("[INFO] 系統走企業高速公路，跳過 QuotaManager 私家金鑰盤查")
        qm = None
    else:
        try:
            qm = QuotaManager()
    ...
    _tw_now = _dt.now(_tz.utc).replace(tzinfo=None) + _td(hours=8)
    _now_hour = _tw_now.hour
    _is_stt_phase = True if ENTERPRISE_MODE else (_now_hour >= 16 or _now_hour < 8)
    ...
    batch = active_tasks[:effective_concurrency * 8]
...
def _run_task_steps(manifest: dict, manifest_path: Path, paths: dict, exclusive_key: str, chunking_only: bool) -> bool:
    ...
    if "source_path" not in manifest and "video_path" in manifest:
        manifest["source_path"] = manifest.get("video_path", "")
        try:
            atomic_json_dump(manifest, manifest_path)
        except Exception:
            pass
    ...
    try:
        # 步驟二：Preprocess
        if manifest["steps"].get("preprocess") == "pending":
        ...
        # 步驟三：Chunk Planner
        if manifest["steps"].get("chunk_planner") == "pending":
        ...
        # ── 接入點 4: Vertex / 外部 API 呼叫層經常是此遊例的根源─────────────
        dump_stack_snapshot(
            task_id=task_id,
            source_name=_source,
