...
def process_single_chunk(
    ...
    while chunk["retry_count"] < retry_limit:
        try:
            ...
            if stt_engine in ["gemini", "vertexai"]:
                ...
                if not uploaded_file:
                    if stt_engine == "gemini":
                        ...
                        v_project = config.get("api", {}).get("vertexai_project")
                        if v_project == "[REDACTED_PROJECT_ID]" or not v_project:
                            v_project = adc_project
                        v_loc = config.get("api", {}).get("vertexai_location", "us-central1")
                        fresh_client = genai.Client(vertexai=True, project=v_project, location=v_loc, credentials=credentials)
                        use_vertex = v_project
                ...
                if fallback_to_gcs:
                    ...
                    if not bucket.exists():
                        bucket = storage_client.create_bucket(f"lexmind-temp-{use_vertex}", location=config.get("api", {}).get("vertexai_location", "us-central1"))
                ...
                try:
                    ...
                    if local_whisper_text and transcription:
                        ...
                        atomic_write_json(chunk_distill_path, {
                            "task_id": task_id,
            ...
            atomic_write_json(json_output_path, {
                "task_id": task_id,
            ...
            if stt_engine == "gemini" and isinstance(e, (google_exceptions.TooManyRequests, 
...
def run_stt(task_id: str, exclusive_key: str = None):
    ...
    stt_engine = config.get("settings", {}).get("stt_engine", "vertexai")
    ...
    if stt_engine == "gemini":
        ...
        v_cred_path = config.get("api", {}).get("vertexai_credentials_path", "config/vertex_key.json")
        ...
        if os.path.exists(cred_abs):
            ...
            logging.info(f"[STT] 未偵測到 {cred_abs}，將直接依賴原生 ADC 認證。")
            import google.auth
            try:
                credentials, adc_project = google.auth.default(scopes=['https://www.googleapis.com/auth/cloud-platform'])
            except Exception as e:
                logging.warning(f"[STT] 原生 ADC 取出失敗: {e}")
        
        v_project = config.get("api", {}).get("vertexai_project")
        if v_project == "[REDACTED_PROJECT_ID]" or not v_project:
            try:
                with open(cred_abs, 'r', encoding='utf-8') as f:
                    key_data = json.load(f)
                    v_project = key_data.get("project_id", adc_project)
            except Exception as e:
                v_project = adc_project
    ...
    with ThreadPoolExecutor(max_workers=CHUNK_CONCURRENCY) as executor:
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                with manifest_write_lock:
                    try:
                        atomic_write_json(chunks_manifest_path, chunks)
