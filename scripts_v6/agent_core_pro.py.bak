    ...
    def __call__(self, input: list) -> list:
        ...
        for text in input:
            try:
                import requests
                import os
                key = self.qm.acquire_key() if self.qm else os.environ.get("GEMINI_API_KEY", "")
                res = requests.post(
                ...
                if res.status_code == 200:
                    embeddings.append(res.json().get('embedding', {}).get('values', [0.0]*3072))
                else:
                    embeddings.append([0.0] * 3072)
            except Exception as e:
                print(f"[WARNING] Gemini Embedding Error: {e}")
                embeddings.append([0.0] * 3072)
        return embeddings
...
class LocalLegalAgent:
    def __init__(self, context_role="lawyer", db_path=None):
        ...
        # 檢測與自動升級 384/768-dim 到 3072-dim (以相容 gemini-embedding-2)
        try:
            coll_docs = self.chroma_client.get_collection(name="legal_docs")
            try:
                # 測試 3072 維度查詢是否相容
                coll_docs.query(query_embeddings=[[0.0] * 3072], n_results=1)
            except Exception as e:
                if "dimension" in str(e).lower() or "expecting embedding" in str(e).lower():
                    print("Deleting old legal_docs collection to upgrade to 3072-dim gemini-embedding-2...")
        ...
        try:
            ...
            try:
                # 測試 3072 維度查詢是否相容
                coll_intel.query(query_embeddings=[[0.0] * 3072], n_results=1)
            except Exception as e:
                if "dimension" in str(e).lower() or "expecting embedding" in str(e).lower():
                    print("Deleting old legal_intelligence_vault collection to upgrade to 3072-dim gemini-embedding-2...")
