...
def is_legal_text(text):
    """分析文字內容，利用正則表達式與關鍵字密度判定是否為法律相關內容，拒絕亂抓"""
    if not text:
        return False
    # 1. 檢查正則特徵
...
def load_scraped_history():
    """載入智慧爬蟲記憶檔，支援新舊結構自動相容轉換與遷移"""
    if os.path.exists(HISTORY_FILE):
        try:
            with open(HISTORY_FILE, "r", encoding="utf-8-sig") as f:
                data = json.load(f)
                if isinstance(data, dict):
                    # 如果是舊版本扁平格式，將其升級為結構化歷史
                    if "scraped_questions" not in data:
                        return {
                            "execution_history": [],
                            "scraped_questions": data
                        }
                    return data
        except Exception:
            pass
...
TARGET_URLS = [
    ...
    "https://lawyer.get.com.tw/exam/List.aspx?sFilterType=0&sFilterDate=&sFilter=%E7%89%B9%E8%80%83%E5%9B%9B%E7%AD%89-%E5%8F%B8%E6%B3%95%E4%BA%BA%E5%93%A1",
    "https://www.lawbank.com.tw/exam/",
    "https://twlawbot.com"
]

def run_independent_scraper():
    update_status("running", 5, "正在初始化爬蟲環境與多目標備援清單...")
    headers = {
    ...
    for url in TARGET_URLS:
        update_status("running", 15, f"嘗試連線備援目標: {url[:30]}...")
        try:
            response = requests.get(url, headers=headers, timeout=10)
            if response.status_code != 200:
                print(f"[{url}] 狀態碼異常: {response.status_code}")
                continue
                
            if not any(kw in url.lower() for kw in ["law", "twlaw", "judge", "court", "lexmind"]) and not is_legal_text(response.text):
                continue
                
            soup = BeautifulSoup(response.text, 'html.parser')
            # 擴增彈性標籤庫
            cards = soup.find_all(['article', 'div', 'table', 'tr', 'li'], class_=['exam-card', 'question-item', 'question-content', 'lp-pro-bar', 'table-row', 'list-item'])
            
            if cards:
                question_cards = cards
                success_url = url
                update_status("running", 30, f"連線成功！從 {url[:25]} 取得考題。")
                break
            else:
                print(f"[{url}] 找不到考題區塊，自動切換至下一個備援...")
    ...
    history["execution_history"].append({
        ...
        "skipped_non_legal": skipped_by_non_legal,
        "message": f"同步成功！本次共新同步 {parsed_count} 題司法官、律師考古題。{skipped_msg}"
    })
