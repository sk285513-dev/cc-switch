...
def plan_chunks(task_id):
    ...
    if has_video:
        limit = settings.get("chunk_limit_video_sec", 720) # 12 mins
    else:
        limit = settings.get("chunk_limit_audio_sec", 1200) # 20 mins
    ...
    for idx, (s_time, e_time) in enumerate(chunks_plan):
        ...
        try:
            res = subprocess.run(cut_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, encoding="utf-8", errors="ignore")
