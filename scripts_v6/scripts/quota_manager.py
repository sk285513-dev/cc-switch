...
class QuotaManager:
    def __init__(self, keys_path: str = "config/keys.yaml", policy_path: str = "config/quota_policy.yaml", state_path: str = "config/quota_state.json"):
        ...
        self.lock_path = Path(state_path).with_suffix(".lock")
        self._load_keys()
        self._load_policy()
        self._load_state()
        ...
        if not self.keys:
            ...
            if config_yaml_path.exists():
                try:
                    with open(config_yaml_path, "r", encoding="utf-8") as f:
                        cfg = yaml.safe_load(f)
                    if cfg and "settings" in cfg and cfg.get("settings", {}).get("vertexai_project"):
                        is_vertex = True
                except:
                    pass
            
            if is_vertex:
                self.keys = ["vertex_adc_dummy_key"]
            else:
