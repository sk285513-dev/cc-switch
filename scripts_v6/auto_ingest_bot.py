...
def is_file_content_legal(f_path):
    ...
    # 3. 影音與圖像檔案在尚未進行 ASR/OCR 之前僅以路徑做為防護線，必須在完整路徑中同時包含【法律科目】與【課程特徵】
...
BUFFER_FILE = "C:\\Users\\temp\\antigravity\\LexMind-Omni-法律實務-AI-工作站\\scripts_v6\\test_data\\chosen_paths_buffer.json"
        ...
        for root, dirs, files in os.walk(drive, topdown=True):
            ...
            if has_valid_media and ((folder_has_law and folder_has_course) or file_matches):
                ...
                if abs_path not in valid_folders:
                    ...
                    reason = "資料夾名稱符合" if folder_has_law else "檔案名稱符合"
...
HISTORY_FILE = "C:\\Users\\temp\\antigravity\\LexMind-Omni-法律實務-AI-工作站\\scripts_v6\\test_data\\ingested_history.json"
...
def run_ingest(folders):
    ...
    transcripts_dir = "C:\\Users\\temp\\antigravity\\LexMind-Omni-法律實務-AI-工作站\\scripts_v6\\test_data\\Transcripts"
    ...
    for idx, (disp_name, f_path, parent_folder) in enumerate(pending_files):
        ...
        try:
            ...
            try:
                future = executor.submit(agent.chat, payload_prompt)
                res = future.result(timeout=180) # 3分鐘超時防卡死
            except FuturesTimeoutError:
                print("   [3/3] ❌ 模型推理發生嚴重超時 (180s)，發動 Executor 斬殺與重建機制...")
                executor.shutdown(wait=False, cancel_futures=True)
                executor = ThreadPoolExecutor(max_workers=1)
                raise RuntimeError("大模型推理超時 (Cascading Timeout 斬殺)")
