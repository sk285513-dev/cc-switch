...
from national_exam_rules import (
    SUPPORTED_EXTS, 
    LAW_KEYWORDS, 
    BLACKLIST, 
    TAIWAN_LEGAL_PATTERNS,
    get_exam_score,
    check_text_for_legal_relevance,
    is_file_content_legal
)
from duplicate_guard import is_already_ingested, record_ingestion
...
def get_video_duration_and_size(file_path):
    ...
    try:
        ...
        result = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, timeout=10, creationflags=getattr(subprocess, 'CREATE_NO_WINDOW', 0x08000000))
...
def scan_drives(scan_path=None):
    ...
    for drive in drives:
        ...
        for root, dirs, files in os.walk(drive, topdown=True):
            ...
            # 檢查完整路徑是否含有黑名單，若有直接跳過
            if any(bl in root.lower() for bl in BLACKLIST):
                dirs.clear() # 黑名單目錄，停止遞迴其子目錄
                continue
            ...
            try:
                ...
                for f in os.listdir(root):
                    ...
                    if f_ext in SUPPORTED_EXTS:
                        # 檢查檔案本身是否中黑名單
                        if any(bl in f.lower() for bl in BLACKLIST):
                            print(f"      [⛔ 黑名單攔截] 跳過檔案 {f}")
                            continue
            ...
            if has_valid_media and (folder_matches or file_matches):
                ...
                if abs_path not in valid_folders:
                    ...
                    priority = get_exam_score(abs_path)
                    print(f"      [找到有效教材資料夾 ({reason}, 優先級: {priority}, 深度: {depth})] {abs_path}")
                        
    # 依據國考核心權重排序模組（憲法10分、民法20分等優先級邏輯）進行排序
    valid_folders.sort(key=lambda x: get_exam_score(x))
    return valid_folders

# [Block E] 編碼保護與原子寫入
def atomic_json_dump(data, filepath):
    tmp_path = str(filepath) + f".{os.getpid()}.{time.time()}.tmp"
...
def is_file_already_ingested(f_path, history):
    ...
    # 2. [防重複處理邏輯] 檢查原始課程檔案的目錄底下看是不是有存在原本的 srt, md, json 檔
    ...
    try:
        ...
        if has_md and has_srt and has_json:
            print(f"      [原始目錄已存在產物] {stem} 在來源資料夾內已有完整的 md, srt, json。判定為已處理。")
            return True
    except Exception:
        pass

    # 3. 若實體檔案不存在，退回檢查 JSON 紀錄檔
...
def run_ingest(folders):
    ...
    for disp_name, f_path, parent_folder in valid_files_info:
        is_processed = is_already_ingested(f_path)
    ...
    for idx, (disp_name, f_path, parent_folder) in enumerate(pending_files):
        ...
        try:
            ...
            if p_file.suffix.lower() in ['.mp3', '.mp4', '.wav']:
                ...
                print(f"   [⚠️ 警告] 不支援的檔案格式 {p_file.suffix}，略過處理。")
                continue
            ...
            if doc_embedding:
                ...
                record_ingestion(f_path, "auto_ingest_bot")
