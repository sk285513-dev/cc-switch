...
# Downstream: None
# Shared State: KPI Data, Dashboard DB
...
# ==========================================
# [EXPERT FIX] 自動識別 V6 隔離環境，切換監聽目錄
# ==========================================
import os
...
def render_rich(tasks, fmt_events, export=False):
    ...
    try:
        ...
        if _kpi_state.exists():
            _ks = _js.loads(_kpi_state.read_text(encoding="utf-8", errors="replace"))
        ...
        for _mf in _manifests.glob("task_*.json"):
            ...
            try:
                _m = _js.loads(_mf.read_text(encoding="utf-8", errors="replace"))
