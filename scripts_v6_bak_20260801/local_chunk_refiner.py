...
def refine_task(task_id, model="deepseek-r1:32b"):
    ...
    try:
        ...
        subprocess.run([sys.executable, "scripts/merge_transcript.py", "--task-id", task_id], cwd="C:/LocalAI_Workstation", check=True, creationflags=0x08000000)
        
        print(">> 執行 markdown_formatter.py...")
        subprocess.run([sys.executable, "scripts/markdown_formatter.py", "--task-id", task_id], cwd="C:/LocalAI_Workstation", check=True, creationflags=0x08000000)
