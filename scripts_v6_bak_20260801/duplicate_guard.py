...
def check_legacy_processed(file_path: str) -> bool:
    ...
    try:
        ...
        for srt_file in processed_dir.glob(f"*{base_name}*.srt"):
            ...
            try:
                res = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, timeout=10, creationflags=getattr(subprocess, 'CREATE_NO_WINDOW', 0x08000000))
