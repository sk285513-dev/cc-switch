...
- **SA 系統分析 (System Analysis & Architecture)**：
  捨棄透過 VBScript 試圖突破 Session 0 的幻想（由於 Windows 核心安全機制，Session 0 的子進程依然受限於背景）。我們改採「雙進程生產者-消費者架構」：背景沙箱或 Task Scheduler 僅負責將測試任務推播至本地佇列 (JSON 搭配純 Windows 原生 KernelMutex 鎖，捨棄任何外部記憶體資料庫的依賴)；而實體桌面 (Session 1) 則常駐一個輕量級的 Watchdog Agent 輪詢該佇列。當偵測到新任務時，Watchdog 在本地實體桌面喚醒 Playwright 進行視覺截圖。這徹底迴避了 `0x800700E8` 崩潰，並達成 100% 可見的所見即所得 (WYSIWYG) 測試。
  ### 4.0 核心機制：Windows 原生 IPC 與 KernelMutex 鎖定 (Native Windows IPC Design)
  系統捨棄了第三方依賴，完全基於 Windows 原生 API 實現跨行程通訊 (IPC)。在 `config/quota_state.json` 或本機佇列存取時，採用 `ctypes.windll.kernel32.CreateMutexW` 建立全域互斥鎖 (Global Mutex)。
  - **鎖定與超時機制**：呼叫 `WaitForSingleObject` 取得控制權，若超過 5000ms 則觸發 TimeoutException 以防死鎖 (Deadlock)。
  - **Abandoned Mutex 處理**：當進程意外崩潰 (如 OOM 遭強制終止)，Windows 核心會回傳 `WAIT_ABANDONED`，我們的 `LockManager` 會自動攔截此訊號，將崩潰的鎖收回並重置狀態，確保佇列永不停擺。
  - **IPC 資料佈局 (Data Layout)**：使用 atomic file rename 模式 (寫入 `.tmp` 後 `os.replace` 原檔)，確保高頻讀寫時消費者絕對不會讀到破損的 JSON 字串。
    ```mermaid
    sequenceDiagram
...
## 4.5 案例研究：動態金鑰管理設定介面 (Settings View) 之極端邊界測試
