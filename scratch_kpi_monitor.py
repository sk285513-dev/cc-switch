...
def log(msg: str):
    ...
    try:
        with open(KPI_LOG, "a", encoding="utf-8-sig") as f:
            f.write(line + "\n")
    except Exception:
        pass
...
def load_snapshot() -> dict:
    if KPI_STATE.exists():
        try:
            return json.loads(KPI_STATE.read_text(encoding="utf-8-sig"))
        except Exception:
            pass
    return {}
...
def save_snapshot(snap: dict):
    KPI_STATE.write_text(json.dumps(snap, ensure_ascii=False, indent=2), encoding="utf-8-sig")
...
def scan_manifests() -> dict:
    ...
    task_appear = Counter()
    try:
        lines = LOG_PATH.read_text(encoding="utf-8-sig", errors="replace").splitlines()[-200:]
        for line in lines:
    ...
    for mf in MANIFESTS.glob("task_*.json"):
        ...
        try:
            ...
            if os.path.exists(cm):
                try:
                    chunks = json.loads(open(cm, encoding="utf-8-sig").read())
...
def restart_workflow():
    ...
    subprocess.run(
        ...
        capture_output=True, timeout=30, cwd=str(WORK_DIR), creationflags=0x08000000
    )
...
def reset_quota_state():
    ...
    QUOTA_STATE.write_text('{"exhausted_keys":[]}', encoding="utf-8-sig")
        ...
        for mf in MANIFESTS.glob(f"*{tid}*.json"):
            ...
            try:
                ...
                if os.path.exists(chunk_file):
                    try:
                        chunks = json.loads(open(chunk_file, encoding="utf-8-sig").read())
                ...
                mf.write_text(json.dumps(m, ensure_ascii=False, indent=2), encoding="utf-8-sig")
...
def run_kpi_check():
    # 根據論文 6.2 節規範，嚴格使用 utcnow() 取得台灣時間
    _tw = datetime.datetime.utcnow() + datetime.timedelta(hours=8)
    now = _tw
    is_stt_phase = now.hour >= STT_PHASE_HOUR or now.hour < 8   # 台灣 16:00-08:00 為 STT
