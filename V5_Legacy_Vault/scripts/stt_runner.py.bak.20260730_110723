...
try:
    with open(r'C:\LocalAI_Workstation\config.yaml', 'r', encoding='utf-8') as _f:
        _cfg = __import__('yaml').safe_load(_f)
        CHUNK_CONCURRENCY = _cfg.get("dispatcher", {}).get("stt_concurrency", 6)
except Exception:
    CHUNK_CONCURRENCY = 6  # 預設為 6（Vertex AI 高並發通道）
...
def process_single_chunk(
    ...
    while chunk["retry_count"] < retry_limit:
        try:
            ...
            with manifest_write_lock:
                try:
                    with open(chunks_manifest_path, "r", encoding="utf-8") as f:
                        curr_chunks = json.load(f)
                    for c in curr_chunks:
                        if c["filename"] == chunk_filename:
                            c["status"] = "processing"
                            c["started_at"] = chunk["started_at"]
                            break
                    with open(chunks_manifest_path, "w", encoding="utf-8") as f:
                        json.dump(curr_chunks, f, ensure_ascii=False, indent=2)
                except Exception:
                    pass
            ...
            err_str = str(e)
            if stt_engine in ["gemini", "vertexai"] and (
