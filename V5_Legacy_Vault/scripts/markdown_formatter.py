...
def parse_transcript_into_timed_segments(text, chunk_start_time, chunk_end_time):
    pattern = r"\[(?:(\d{1,2}):)?(\d{2}):(\d{2})\]"
    matches = list(re.finditer(pattern, text))
    
    segments = []
    if not matches:
        # 若找不到時間戳，依照字串長度與總時間 (預設 360 秒或實際區間) 平均分割
        lines = [line.strip() for line in re.split(r'[。！？\n]', text) if line.strip()]
        if not lines:
            return [{"start": chunk_start_time, "end": chunk_end_time, "text": text}]
        
        duration = chunk_end_time - chunk_start_time
        if duration <= 0:
            duration = 360.0
            
        avg_len = duration / len(lines)
        for idx, line in enumerate(lines):
            segments.append({
                "start": chunk_start_time + idx * avg_len,
                "end": chunk_start_time + (idx + 1) * avg_len,
                "text": line
            })
        return segments
        
    # 依實際音檔長度等比例縮放時間軸，解決 LLM (Gemini) 的時間戳幻覺與漂移
    raw_seconds = [parse_timestamp_to_seconds(m) for m in matches]
    max_raw = max(raw_seconds) if raw_seconds else 1.0
    if max_raw <= 0: max_raw = 1.0
    
    actual_duration = chunk_end_time - chunk_start_time
    if actual_duration <= 0: actual_duration = 360.0
    
    # 估算最後一個段落所需的語音時間 (假設每秒 4 個字元)
    last_idx = matches[-1].end()
    last_text = text[last_idx:].strip()
    last_text_duration = max(5.0, len(last_text) / 4.0)
    
    estimated_total_raw = max_raw + last_text_duration
    scale_factor = actual_duration / estimated_total_raw
    
    for i in range(len(matches)):
        m = matches[i]
        raw_sec = parse_timestamp_to_seconds(m)
        scaled_sec = raw_sec * scale_factor
        start_sec = scaled_sec + chunk_start_time
    ...
    if segments:
        segments[-1]["end"] = min(segments[-1]["start"] + (last_text_duration * scale_factor), chunk_end_time)
...
"""
    ...
    for idx, chunk in enumerate(chunks):
        ...
        chunk_segs = parse_transcript_into_timed_segments(chunk_content, chunk_start, chunk_end)
    ...
    # 11. 備份 chunks 目錄至 F:\ 或 E:\ (若 F 空間小於 50GB)
    import shutil
    try:
        f_drive = "F:\\chunks_backup"
        e_drive = "E:\\chunks_backup"
        target_dir = f_drive
        ...
        backup_path = os.path.join(target_dir, task_id)
        
        if os.path.exists(task_chunks_dir):
            if os.path.exists(backup_path):
                shutil.rmtree(backup_path) # 清除舊的同名備份
            shutil.copytree(task_chunks_dir, backup_path, dirs_exist_ok=True)
            log_workflow(f"Markdown Formatter: Successfully backed up chunks to {backup_path}")
    except Exception as backup_err:
        log_error(f"Markdown Formatter: Failed to backup chunks: {backup_err}")
