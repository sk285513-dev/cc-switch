...
if V6_ENV == "v6_canary":
    ...
    LOG_PATH = Path("A:/logs_v6/workflow.log")
else:
...
def load_all_tasks():
    ...
    if processed_dir.exists():
        for md_file in processed_dir.glob("*.md"):
            ...
            import hashlib
            hash_id = hashlib.md5(folder_name.encode('utf-8')).hexdigest()[:8]
            task_id = f"hist_{hash_id}"
...
def render_rich(tasks, fmt_events, export=False):
    ...
    for t in done_tasks:
        t["fmt_ts"] = fmt_events.get(t["task_id"]) or t.get("hist_ts")
    ...
    if active:
        ...
        def sort_key(t):
            ...
            try:
                import national_exam_rules
                priority = national_exam_rules.get_exam_score(t.get("name", ""))
            except Exception:
                priority = 100
    ...
    if active_chunks:
        ...
        for ac in active_chunks:
            mins, secs = divmod(int(ac['elapsed']), 60)
            tbl_chunks.add_row(ac['filename'], f"{mins:02d}:{secs:02d}")
        console.print(tbl_chunks)
...
def render_plain(tasks, fmt_events):
    ...
    print("\n[ 已完成課程 - 最近 15 筆 ]")
    with_ts = [(t, fmt_events.get(t["task_id"]) or t.get("hist_ts")) for t in tasks if t["status"]=="completed" and (fmt_events.get(t["task_id"]) or t.get("hist_ts"))]
    with_ts.sort(key=lambda x: x[1], reverse=True)
