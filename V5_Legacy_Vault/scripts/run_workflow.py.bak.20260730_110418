...
if V6_ENV == "v6_canary":
    LOGS_DIR = Path("A:/logs_v6")
    PROCESSED_DIR = Path("A:/processed_md_v6")
else:
...
def load_config_for_concurrency():
    try:
        with open(r'C:\LocalAI_Workstation\config.yaml', 'r', encoding='utf-8') as f:
            return yaml.safe_load(f)
    except Exception:
        return {}

cfg = load_config_for_concurrency()
LOCAL_INITIAL = cfg.get("dispatcher", {}).get("max_workers", 20)
LOCAL_MIN = 12

class ElasticConcurrencyManager:
    def __init__(self, initial, min_val):
        self.current = initial
        self.min_val = min_val
        self.lock = threading.Lock()

    def step_down(self, task_id, error_msg):
        with self.lock:
            if self.current > self.min_val:
                self.current -= 1
                log_error(f"[Elastic Concurrency] 任務 {task_id} 發生錯誤 ({error_msg})。本地並發上限自動降級為: {self.current}")

elastic_manager = ElasticConcurrencyManager(LOCAL_INITIAL, LOCAL_MIN)
...
def run_single_task_safe(manifest: dict, manifest_path: Path, exclusive_key: str, qm: QuotaManager, chunking_only: bool) -> bool:
    ...
    try:
        ...
        if "mock_" in source_name or "lexmind_mock_" in source_name:
            success = _run_task_steps(manifest, manifest_path, paths, exclusive_key, chunking_only)
        else:
            success = _run_task_steps(manifest, manifest_path, paths, exclusive_key, chunking_only)
            
        if not success:
            elastic_manager.step_down(task_id, "步驟回報失敗")
        return success
    except Exception as e:
        log_error(f"[Parallel Dispatcher] Task {task_id} 發生異常：{e}")
        elastic_manager.step_down(task_id, str(e))
        try:
...
def process_active_tasks(chunking_only=False):
    ...
    # [End-to-End Test] 僅限定執行排在最前面的三個課程任務
    ...
    # ── 并列分派 ──
    with ThreadPoolExecutor(max_workers=elastic_manager.current) as executor:
        futures = {}
