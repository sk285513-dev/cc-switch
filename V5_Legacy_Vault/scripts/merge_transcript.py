...
def call_gemini_api(prompt, model_name, api_key):
    ...
    # 定義嚴格的同層級備選表格 (Tiered Fallback Matrix)
    # 限制模型只能在自己所屬的「成本與性能層級」內進行退避，絕對禁止跨層級 (如 Flash 跳到 Pro)
    FALLBACK_MATRIX = {
        "flash": [
            "gemini-2.5-flash",
            "gemini-2.0-flash",
    ...
    # 根據輸入的模型名稱判斷所屬等級 (tier)
    tier = "pro" if "pro" in model_name.lower() else "flash"
    
    # 嚴格限制：只能從同層級 (tier) 的表格中加入備用模型
    for m in FALLBACK_MATRIX[tier]:
...
def merge_workflow(task_id):
    ...
    # [GUARD 1] 確保 manifest 讀取成功且不為空
    if not manifest:
        log_error("Merge Agent: [GUARD] manifest 讀取失敗或內容為空，拒絕進入 merge")
        return False
        
    # [GUARD 2] 確保 chunks 清單存在且不為空
    if not chunks:
        log_error("Merge Agent: [GUARD] chunks 清單為空 (0 個切片)，拒絕合併")
        return False

    # [GUARD 3] 防禦 STT 完成率不足與 0/0 錯誤
    total_chunks = len(chunks)
    completed_chunks = sum(1 for c in chunks if c.get("status") == "completed")
    completion_rate = completed_chunks / total_chunks

    if completion_rate < 0.85:
        log_error(
            f"Merge Agent: [GUARD] STT 完成率不足！"
            f"{completed_chunks}/{total_chunks} = {completion_rate:.0%} < 85%，拒絕合併..."
        )
        return False
    ...
    # 嚴格讀取系統設定的 merge_model，尊重使用者的設定精靈選項，不可 hardcode
    model_name = config.get("api", {}).get("merge_model", "gemini-2.5-flash")
    ...
    for idx, chunk in enumerate(chunks):
        ...
        if api_key:
            ...
            try:
                block_summary = call_gemini_api(PROMPT_SUMMARY.format(text=chunk_content), model_name, api_key)
    ...
    # 更新任務 Manifest（此區段只在 merge 真正成功時執行）
