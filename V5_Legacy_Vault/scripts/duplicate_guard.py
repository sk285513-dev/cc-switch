...
def check_source_directory_processed(file_path: str) -> bool:
    """檢查原始檔案所在目錄是否已經有轉寫成功的完整備份檔"""
    try:
        path_obj = Path(file_path)
        base_name = path_obj.stem
        parent_dir = path_obj.parent
        
        # 依照使用者指示，需嚴格確認這 5 種檔案
        required_exts = [".md", ".srt", ".txt", ".vtt", "_index.json"]
        found_exts = []
        
        for ext in required_exts:
            for backup_file in parent_dir.glob(f"*{base_name}*{ext}"):
        ...
        # 只要最核心的 .md 或 .srt 存在，就絕對不能重複抓取去燒錢重作
        if ".md" in found_exts or ".srt" in found_exts:
            if len(found_exts) == 5:
                log_workflow(f"[Source Backup Guard] 完美防護：原始目錄五大成品皆已齊全，阻擋重複抓取。")
            else:
                log_workflow(f"[Source Backup Guard] 發現原始目錄已有核心成品 {found_exts}，阻擋重複抓取。")
            return True
            
        return False
    except Exception as e:
        log_error(f"Error checking source directory for {file_path}: {e}")
        return False
...
def is_already_ingested(file_path: str, auto_migrate: bool = True) -> bool:
    ...
    try:
        ...
        # [Bug #6 Fix] 檢查原始目錄是否有成品備份
        if check_source_directory_processed(file_path):
            log_workflow(f"[Duplicate Guard] 發現原始目錄已有成品備份，自動補登帳本: {os.path.basename(file_path)}")
            record_ingestion(file_path, "source_backup_recovery")
            return True
