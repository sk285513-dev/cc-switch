...
# We need to append the exact code blocks to the end of the file.
code_blocks = """

### 3.1 [MODIFY] `scripts_v6/merge_transcript.py`
請將檔案中原有的 `def merge_workflow(task_id):` 函數**整段替換**為以下安全防護版本，並清除檔案頂部重複的 `sys`, `json`, `time`, `Path`, `io` 引用：
...
### 3.2 [MODIFY] `restart_workflow.ps1`
請將整支檔案替換為以下加入 WMI 反查機制的「驗證式重啟」版本：
...
if (-not $wf) {
    ...
    if (Test-Path $stderrLog) {
        Write-Host "\\n--- stderr tail ---" -ForegroundColor Yellow
        Get-Content $stderrLog -Tail 30 -Encoding UTF8
    }

    if (Test-Path $stdoutLog) {
        Write-Host "\\n--- stdout tail ---" -ForegroundColor Yellow
...
print('Plan appended successfully.')
