...
def render_settings_page():
    ...
    with col_btn2:
        test_clicked = st.button(_("📥 2. 從背景同步金鑰存活狀態 (True IPC)"))
    with col_btn3:
    ...
    if test_clicked and gemini_keys:
        ...
        st.info("從背景 QuotaManager 同步金鑰狀態...")
        state_file = Path("config/quota_state.json")
        exhausted_hashes = []
        if state_file.exists():
            try:
                with open(state_file, "r", encoding="utf-8") as f:
                    state = json.load(f)
                exhausted_hashes = state.get("exhausted_keys", [])
            except:
                pass
        ...
        for i, k in enumerate(gemini_keys):
            ...
            key_hash = hashlib.sha256(val.encode('utf-8')).hexdigest()
            if key_hash in exhausted_hashes:
                status_str = "🔴 背景標記為失效 (401/403/429)"
            ...
            if isinstance(k, dict):
                ...
                gemini_keys[i] = {"account": "", "value": val, "active": (key_hash not in exhausted_hashes), "notes": status_str, "added_date": "", "expiry_date": ""}
            my_bar.progress((i+1)/total)
        st.success("狀態同步完畢！請確認後點擊最下方「儲存並套用設定」。")
    ...
    if submitted:
        ...
        # 【資安修補：記憶體抹除】 將字串轉為 bytearray，存入 Vault 後手動將其抹除
        raw_keys_bytes = [bytearray(str(k.get("value")).encode('utf-8')) for k in new_keys if k.get("value")]
        
        mutex = KernelMutex("Global\\LexMind_Vault_Mutex")
        locked = mutex.acquire(15000)
        if locked:
            try:
                v = Vault()
                # 轉回 string 供 Vault 加密 (Vault 內部預期 string 或會自行處理)
                string_keys = [b.decode('utf-8') for b in raw_keys_bytes]
                v.encrypt_and_save(string_keys)
                
                # 安全清除記憶體中的明文
                for i in range(len(string_keys)):
                    string_keys[i] = "0" * len(string_keys[i])
                for b in raw_keys_bytes:
                    b[:] = b'\x00' * len(b)
            finally:
                mutex.release()
                mutex.close()
        
        # 不要將 gemini_keys 寫入明文配置
        ...
        if save_settings(new_config):
            if not locked:
                st.warning(_("設定已儲存，但金鑰庫鎖定逾時，未能更新金鑰！"))
            else:
                st.success(_("設定與加密金鑰已成功儲存！重新載入中..."))
        else:
