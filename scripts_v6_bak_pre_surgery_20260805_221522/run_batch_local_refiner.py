...
def run_batch():
    ...
    for filename in buggy_filenames:
        ...
        if not os.path.exists(chunks_manifest_path):
            with open(chunks_manifest_path, "w", encoding="utf-8-sig") as f:
                json.dump([], f)
        
        try:
            # 呼叫我們剛剛寫好的完美隔離腳本
            subprocess.run([sys.executable, "-u", "scripts/local_chunk_refiner.py", "--task_id", task_id], cwd="C:/LocalAI_Workstation", check=True, creationflags=0x08000000)
