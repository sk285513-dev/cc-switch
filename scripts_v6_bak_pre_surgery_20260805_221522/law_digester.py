...
class LawDigesterPro:
    def __init__(self, db_path=None):
        ...
        self.intel_coll = self.client.get_or_create_collection(name="legal_intelligence_vault")
        self.embed_url = f'{os.environ.get("OLLAMA_HOST", "http://localhost:11434")}/api/embeddings'
        ...
        for idx, chunk in enumerate(chunks):
            ...
            try:
                ...
                res = requests.post(self.llm_url, 
                                    json={'model': os.environ.get("OLLAMA_MODEL", "deepseek-r1:32b"), 'messages': [{'role': 'user', 'content': prompt}], 'stream': False, 'options': {'num_ctx': 32768}},
                                    timeout=90)
                if res.status_code == 200:
                    digested_text = res.json().get('message', {}).get('content', '')
