...
# ==========================================
# 【機制二十五】非破壞性靜音切片邊界預判演算法
# 目的：使用 ffmpeg silencedetect 在背景進行離線 CPU 分析，找出 >1.5s 且 <-35dB 的靜音點。
# ==========================================
...
def find_silences(wav_path, noise_db=-35, duration=1.5):
    ...
    result = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, encoding="utf-8-sig", errors="ignore", creationflags=getattr(subprocess, 'CREATE_NO_WINDOW', 0x08000000))
    silences = []
...
def plan_chunks(task_id):
    ...
    # 統一所有教材（無論影音）的切片上限為 6 分鐘 (360s)
    # 以防止 Base64 檔案過大塞爆 Gemini API Payload
    limit = settings.get("chunk_limit_video_sec", 360) 
    ...
    for idx, (s_time, e_time) in enumerate(chunks_plan):
        ...
        try:
            res = subprocess.run(cut_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, encoding="utf-8-sig", errors="ignore", creationflags=getattr(subprocess, 'CREATE_NO_WINDOW', 0x08000000))
            ...
            with open(manifest_file, "w", encoding="utf-8-sig") as wf:
                json.dump(manifest, wf, ensure_ascii=True, indent=2)
            return False
        if res.returncode != 0:
    ...
    with open(chunks_manifest_file, "w", encoding="utf-8-sig") as wf:
        json.dump(chunks_list, wf, ensure_ascii=True, indent=2)
