...
def preprocess(task_id):
    ...
    # ══════════════════════════════════════════════════════════════
    MIN_SKIP_SEC  = 2     # 低於此秒數：直接拒絕 (放寬以允許短秒數測試檔)
    ...
    if duration < MIN_SKIP_SEC:
        ...
        # 寫入告警日誌供人工複查
        try:
            with open(_alert_log, "a", encoding="utf-8-sig") as _af:
        ...
        with open(manifest_file, "w", encoding="utf-8-sig") as wf:
            _json.dump(manifest, wf, ensure_ascii=True, indent=2)
    ...
    try:
        result = subprocess.run(ffmpeg_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, encoding="utf-8-sig", errors="ignore", creationflags=getattr(subprocess, 'CREATE_NO_WINDOW', 0x08000000))
