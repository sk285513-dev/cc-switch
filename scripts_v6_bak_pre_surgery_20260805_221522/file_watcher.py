...
def scan_new_files():
    ...
    for raw_path in scan_paths:
        ...
        for root, dirs, files in os.walk(str(raw_path)):
            ...
            for f in files:
                if f.startswith("chunk_") or "temp_extract_" in f:
                    continue
                item = Path(root) / f
                if item.is_file() and item.suffix.lower() in SUPPORTED_EXTENSIONS:
                    ...
                    if "chunk_" in lower_name or "temp_" in lower_name or "extracted_" in lower_name:
                        continue
                    if not is_file_content_legal(abs_item_path):
                        continue
                        
                    if not is_already_ingested(abs_item_path):
                        # 動態影片健康度檢查：取得時長與大小
                        ...
                        if duration_min > 0:
                            ...
                            # 嚴格過濾：改為允許短於 20 分鐘的「測試檔」(放寬至 0.05 分)
                            if duration_min < 0.05 or ratio < lower_bound or ratio > 500.0:
                        ...
                        try:
                            with open(manifest_file, "w", encoding="utf-8-sig") as fw:
                                json.dump(manifest_data, fw, ensure_ascii=True, indent=2)
                            
                            record_ingestion(abs_item_path, "cloud_pipeline")
