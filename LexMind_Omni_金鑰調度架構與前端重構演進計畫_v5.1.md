# 系統分析 (SA)：LexMind-Omni 金鑰調度架構與全端系統重構演進計畫
...
### 1.6 給未來 AI 的鐵律 (The Golden Rule for Future AIs)
...
### 1.7 血淚反省：金鑰迷航與六次沙盒測試的真實除錯歷程 (Blood & Tears: The 6 Sandbox Tests)
...
* **修改後程式碼 (AFTER)**:
  ...
  `` - [x] **2. `FileLock` 的 15 秒 Timeout 未處理導致崩潰** - `[ 實作方式: 自行開發 | 外部相依: 無 (使用內建套件) | 專家諮詢: 無須 ]` - **修改前程式碼 (BEFORE)**: ``python
  ...
  `` - [x] **3. `KeyManager` 解析 `None` 值時引發 KeyError** - `[ 實作方式: 自行開發 | 外部相依: 無 (使用內建套件) | 專家諮詢: 無須 ]` - **修改前程式碼 (BEFORE)**: ``python
  ...
  added\_count += 1
  `` - [x] **4. 純字串比對異常 (`"429" in err_str`) 容易誤判** - `[ 實作方式: 自行開發 | 外部相依: 無 (使用內建套件) | 專家諮詢: 無須 ]` - **修改前程式碼 (BEFORE)**: ``python
  ...
  `` - [x] **5. Library code (`KeyManager`) 中呼叫 `sys.exit(1)`** - `[ 實作方式: 自行開發 | 外部相依: 無 (使用內建套件) | 專家諮詢: 無須 ]` - **修改前程式碼 (BEFORE)**: ``python
  ...
  return []
  `` - [x] **6. SecureMemoryWiper 抹除不可變字串導致崩潰 (記憶體區段錯誤)** - `[ 實作方式: 自行開發 | 外部相依: 無 (使用內建套件) | 專家諮詢: 無須 ]` - **修改前程式碼 (BEFORE)**: ``python
  ...
  key\_bytes.clear()
  `` - [x] **7. Fernet 僅為 AES-128，未達 AES-256 合規標準** - `[ 實作方式: 自行開發 | 外部相依: 無 (使用內建套件) | 專家諮詢: 無須 ]` - **修改前程式碼 (BEFORE)**: ``python
  ...
  `` - [x] **8. KeyManager 呼叫了不存在的 Vault.decrypt_data 靜態方法** - `[ 實作方式: 自行開發 | 外部相依: 無 (使用內建套件) | 專家諮詢: 無須 ]` - **修改前程式碼 (BEFORE)**: ``python
  ...
  vault = Vault()
  decrypted\_content = vault.decrypt\_data(raw\_content)
  `` - [x] **9. 錯誤日誌中試圖遮蔽 Vault 金鑰失敗 (Information Leak)** - `[ 實作方式: 自行開發 | 外部相依: 無 (使用內建套件) | 專家諮詢: 無須 ]` - **修改前程式碼 (BEFORE)**: ``python
...
* **修改後程式碼 (AFTER)**:
  ...
  `` - [x] **20. SRE 可靠性：驚群效應 (Thundering Herd Vulnerability)** - `[ 實作方式: 自行開發 | 外部相依: 無 (使用內建套件) | 專家諮詢: 無須 ]` - **修改前程式碼 (BEFORE)**: ``python
  ...
  `` - [x] **21. 資安：天真的正則表達式謬誤 (Security Theater)** - `[ 實作方式: 自行開發 | 外部相依: 無 (使用內建套件) | 專家諮詢: 無須 ]` - **修改前程式碼 (BEFORE)**: ``python
  ...
  - *(已合併至上述第 6 題處理)*
  - [x] **23. 資安：Vault 缺乏信任根 (Master Key 明文風險)** - `[ 實作方式: 自行開發 | 外部相依: 無 (使用內建套件) | 專家諮詢: 無須 ]` - **修改前程式碼 (BEFORE)**: ``python
  ...
  `` - [x] **24. 資安：容易死鎖的檔案鎖定 (孤兒鎖)** - `[ 實作方式: 自行開發 | 外部相依: 無 (使用內建套件) | 專家諮詢: 無須 ]` - **修改前程式碼 (BEFORE)**: ``python
  ...
  with FileLock(lock\_path, timeout=15):
  `` - [x] **25. UX/UI：透過 UI 重置狀態有缺陷 (測試污染)** - `[ 實作方式: 自行開發 | 外部相依: 無 (使用內建套件) | 專家諮詢: 無須 ]` - **修改前程式碼 (BEFORE)**: ``typescript
  // server.ts 原本只有產品線的 reset-db，容易造成測試與生產環境污染
  app.post("/api/reset-db", (req, res) => { ... })
  `- **修改後程式碼 (AFTER)**:`typescript
  ...
  `` - [x] **26. UX/UI：脆弱的非同步同步機制 (Race Condition)** - `[ 實作方式: 自行開發 | 外部相依: 無 (使用內建套件) | 專家諮詢: 無須 ]` - **修改前程式碼 (BEFORE)**: ``python
  ...
  `` - [x] **27. UX/UI：無視脈絡的涵蓋陣列 (無效測試組合)** - `[ 實作方式: 自行開發 | 外部相依: 無 (使用內建套件) | 專家諮詢: 無須 ]` - **修改前程式碼 (BEFORE)**: ``python
  ...
  continue
  pairs\_needed.add((i, j, vi, vj))
  `` - [x] **28. 學術排版：標題編號不連貫** - `[ 實作方式: 自行開發 | 外部相依: 無 (使用內建套件) | 專家諮詢: 無須 ]` - **修改前程式碼 (BEFORE)**: ``markdown
  ...
  `` - [x] **29. 學術排版：附錄的明顯重複與錯字** - `[ 實作方式: 自行開發 | 外部相依: 無 (使用內建套件) | 專家諮詢: 無須 ]` - **修改前程式碼 (BEFORE)**: ``markdown
  ...
  `` - [x] **30. 學術排版：章節結構失衡** - `[ 實作方式: 自行開發 | 外部相依: 無 (使用內建套件) | 專家諮詢: 無須 ]` - **修改前程式碼 (BEFORE)**: ``markdown
...
為解決 AI 代理人「因自然語言指令差異而找不到對應腳本」的歷史痛點，本章節將整座「工廠」的系統架構以心智圖與索引表格呈現。未來 AI 必須透過此對照表，精準將使用者的「口語化指令 (Tags)」映射到正確的實體腳本。

### 3.1 系統架構心智圖 (Mermaid Mind Map)
...
### 3.2 系統腳本功能與口語化索引對照表 (Script Semantic Tags & Inventory)

此表格列出了所有核心腳本的確切檔名、功能定義，以及未來使用者可能使用的「口語化標籤 (Tags)」。AI 必須根據 Tags 反查此表。

| 系統區塊 (Domain) | 實體腳本名稱 (Script) | 核心功能定義 (Core Function) | 🗣️ 使用者口語化標籤 (User Prompt Tags) |
| :--- | :--- | :--- | :--- |
| **原材料物流 (Ingestion)** | `auto_ingest_bot.py` | 掃描硬碟，嚴格過濾 `.mp3/.mp4/.wav`，阻擋雜訊並依國考權重匯入新影片。 | `找教材`、`去吸教材`、`自動找課`、`抓課程`、`過濾新影片`、`批次處理教材`、`消化課程` |
| **題庫訓練 (Training)** | `batch_exam_trainer.py` | 批次將法律題庫與解答匯入，進行針對性的 LLM 國考對抗訓練。 | `題庫訓練`、`跑題庫`、`批次訓練腳本`、`考題吸收`、`測驗訓練` |
| **物流防護 (Guard)** | `duplicate_guard.py` | 確保已經轉錄過或存在 `processed_md` 的檔案不會被重複送入管線。 | `防重複`、`避免重複抓`、`已經做過的不要再做` |
| **管線中樞 (Orchestration)** | `run_workflow.py` | 系統的總指揮官。負責調度切片、轉寫、合併排版的執行緒與先後順序。 | `總管線`、`主程式`、`派工腳本`、`控制流程`、`執行緒派發` |
| **領域規則核心 (Domain Rules)**| `national_exam_rules.py` | 系統的單一真相來源 (SSOT)。統一收納所有法規白名單、黑名單、與國考優先權重(10分/20分)打分邏輯，供前端與題庫模組共同調用，避免邏輯分裂。 | `改權重`、`改分數`、`改白名單`、`過濾規則`、`核心打分`、`黑名單管理` |
| **媒體預處理 (Pre-process)** | `preprocess_media.py` | 呼叫 ffmpeg 進行靜音偵測，將大影片切成 12 分鐘的音軌碎片。 | `切片`、`切音檔`、`影片預處理`、`抓靜音`、`轉wav` |
| **轉錄引擎 (STT Engine)** | `stt_runner.py` | 呼叫 Gemini Flash 模型進行語音聽寫，附帶 429 防爆與金鑰輪替機制。 | `語音轉文字`、`打逐字稿`、`聽寫`、`字幕產出`、`STT` |
| **排版引擎 (Formatter)** | `markdown_formatter.py`| 將逐字稿與板書截圖融合，自動產生標題階層，輸出最終 `.md`。 | `排版`、`整理講義`、`產出MD`、`插入板書`、`最後的輸出` |
| **大腦與知識庫 (RAG)** | `law_digester.py` | 將講義文本進行 Chunking 並寫入 ChromaDB / Qdrant 向量資料庫。 | `做向量`、`建知識庫`、`把講義吃進大腦`、`切塊存庫` |
| **金鑰保險箱 (Key Vault)** | `quota_manager.py` | 儲存並加密金鑰池，實作斷路器防止 429 / 403 限流導致系統癱瘓。 | `管金鑰的`、`換 API Key`、`429修復`、`金鑰輪替`、`保險箱` |
| **系統守護者 (SRE)** | `sre_watchdog.py` | 背景監控進程，若發現管線卡死或溫度過高，自動獵殺殭屍進程並重啟。 | `守護神`、`重啟腳本`、`看門狗`、`清殭屍`、`防當機` |
| **前端戰情室 (UI / View)** | `app.py` / `settings_view.py` | 供使用者操作的視覺化儀表板，顯示轉譯進度與金鑰消耗狀態。 | `儀表板`、`UI介面`、`首頁`、`設定頁面`、`看進度的面板` |
