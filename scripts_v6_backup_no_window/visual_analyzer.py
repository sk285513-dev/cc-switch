...
# Model Router Agent — 模型切換全權由此模組負責
try:
    from model_router import rest_generate as _router_rest_generate
    from model_router import sdk_generate_image as _router_sdk_generate_image
    _HAS_ROUTER = True
except ImportError:
    _HAS_ROUTER = False
...
class VisualLegalAnalyzer:
    def __init__(self, agent_instance=None):
        ...
        self.llm_url = f'{os.environ.get("OLLAMA_HOST", "http://127.0.0.1:11434")}/api/chat'
        self.model_name = os.environ.get("OLLAMA_MODEL", "deepseek-r1:7b")
        # 依使用者要求強制關閉雲端影像上傳，確保只送出本地錄音檔，影像留在本地用 Tesseract/Ollama
    ...
    def _query_gemini_multimodal(self, image_path, prompt, qm, api_key):
        consecutive_429 = 0
        attempt = 0
        while True:
            try:
                ...
                if _HAS_ROUTER:
                    try:
                        cfg = load_config()
                        stt_engine = cfg.get("settings", {}).get("stt_engine", "gemini")
                        use_vertex = (stt_engine == "vertexai")
                        v_project = cfg.get("settings", {}).get("vertexai_project")
                        v_loc = cfg.get("settings", {}).get("vertexai_location")
                        v_cred = cfg.get("settings", {}).get("vertexai_credentials_path")
                        if v_cred:
                            import os
                            v_cred = os.path.normpath(os.path.join(cfg.get("paths", {}).get("project_root", "A:\\"), v_cred))
                            
                        if qm and not use_vertex:
                            qm.throttle_key(api_key)

                        content = _router_sdk_generate_image(
                    ...
                    # Fallback
                    print("[Gemini OCR] 缺少 ModelRouter，無法執行。")
                    return "", api_key
                    
                return (content.strip() if content else ""), api_key
...
- **校對筆記**: 
"""
                with open(note_path, 'w', encoding='utf-8-sig') as f_note:
    ...
    def track_video_blackboard(self, video_path: str, class_name: str, lesson_name: str, status_callback=None) -> list:
        ...
        if QuotaManager:
            qm = QuotaManager()
            try:
                gemini_key = qm.acquire_key_exclusive()
            except Exception as e:
                print(f"[WARNING] VisualAnalyzer QuotaManager 無法取得排他鎖: {e}")
                gemini_key = None
        ...
        try:
            ...
            interval_sec = 15 # 預設 15 秒 (3x 速度提升)
            try:
                cfg = load_config()
                interval_sec = cfg.get("settings", {}).get("video_sample_interval_sec", 15)
            except:
                pass
                
            sample_interval = interval_sec * fps
            ...
            # 板書追蹤最長允許時間（秒），防止 formatter 永久卡死
            import time as _time
            _bb_timeout = int(os.environ.get("BLACKBOARD_TIMEOUT_SEC", "300"))
            _bb_start = _time.time()
    
            while True:
                if frame_idx >= max_test_frames:
                    break
                # 硬性逾時保護
                if _time.time() - _bb_start > _bb_timeout:
                    import logging
                    logging.warning(f"[VisualAnalyzer] 板書追蹤逾時 {_bb_timeout}s，提前結束（已處理 {frame_idx} 幀）")
                    break
    ...
    """
                                    ai_response, gemini_key = self._query_gemini_multimodal(str(board_path), gemini_prompt, qm, gemini_key)
    ...
    """
            ...
            if qm and gemini_key:
                qm.release_key(gemini_key)
