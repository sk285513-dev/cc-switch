...
"""
    ...
    # 檢查分片大小是否超過雲端上傳極限 (2GB)
    file_size = os.path.getsize(chunk_path)
    if file_size > 2000 * 1024 * 1024:
        raise ValueError(f"分片檔案 {chunk_filename} 大小 ({file_size / (1024**3):.2f} GB) 超過雲端 Gemini Files API 上傳上限 2GB！請在本機設定更小的切片限制。")
...
def run_stt(task_id):
    ...
    # Save the updated chunks list back to the chunks manifest file
