...
class AutoHealer:
    ...
    def escalate_to_human(self, signature, message):
        now = time.time()
        ...
        logging.error(f"[ESCALATION] {signature} - {message}")
        try:
            flags = MB_OK | MB_ICONERROR | MB_SERVICE_NOTIFICATION | 0x40000
            ctypes.windll.user32.MessageBoxTimeoutW(0, ctypes.c_wchar_p(message), ctypes.c_wchar_p(title), flags, 0, 30000)
    ...
    def process_incident(self, incident):
        ...
        if self.heal_attempts[signature] > 3:
            self.escalate_to_human(signature, f"自動修復失敗超過 3 次！\n特徵: {signature}\n請手動介入檢查。")
