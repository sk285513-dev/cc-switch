...
from scripts.markdown_formatter import format_markdown
from scripts.quota_manager import QuotaManager
from scripts.win32_kernel import KernelMutex, KernelEvent, KernelJobObject
from scripts.security_utils import setup_secure_logger
from workflow_helper import ensure_dirs, log_workflow, log_error

# Phase 7.10: 啟動全域資安防護 (攔截 Exception 中的明文金鑰)
setup_secure_logger()

# === Phase 7.8 核心物件 === "Global\\LexMind_Heartbeat_Event"
HEARTBEAT_EVENT_NAME = "Global\\LexMind_Heartbeat_Event"
JOB_NAME = "Global\\LexMind_Job"

# Phase 7.8: Initialize global event for heartbeats
heartbeat_event = None
try:
    heartbeat_event = KernelEvent(HEARTBEAT_EVENT_NAME)
except Exception:
    pass
...
class SingleInstanceLock:
    def __init__(self, lock_file):
        # Phase 7.8: 改用核心物件 KernelMutex
        self.mutex_name = "Global\\LexMind_Workflow_Lock"
        self.mutex = None

    def acquire(self):
        try:
            self.mutex = KernelMutex(self.mutex_name)
            # 等待 0 毫秒，若已被佔用則立刻返回 False
            return self.mutex.acquire(0)
        except Exception:
            return False
...
def get_legal_priority_score(source_name):
    ...
    # 1. 國考核心實體法（絕對優先）：民法、刑法、行政法、國家賠償
    core_substantive_keywords = ["民法", "刑法", "行政程序", "行政法", "國家賠償", "國賠"]
    for kw in core_substantive_keywords:
        if kw in name:
            return 10
            
    # 2. 程序法次之：民訴、刑訴、行訴、強執、家事事件等
    procedural_keywords = ["民事訴訟", "民訴", "刑事訴訟", "刑訴", "行政訴訟", "強制執行", "家事事件", "程序法"]
    for kw in procedural_keywords:
        if kw in name:
            return 20
            
    # 3. 土地法規與地政專業科目
    land_keywords = ["土地登記", "土登", "土地法", "土地稅", "地價稅", "土地法規"]
    for kw in land_keywords:
        if kw in name:
            return 30
            
    # 4. 其他商事法 (公司、票據、保險、海商、信託等)
    commercial_keywords = ["公司法", "票據", "保險法", "海商", "信託法", "勞動基準", "勞基"]
    for kw in commercial_keywords:
        if kw in name:
            return 40
            
    # 5. 其他分科特別法 (智財、消保、公平交易等)
    special_keywords = ["專利", "著作權", "商標", "消費者保護", "消保", "公平交易", "營業秘密", "個人資料", "個資", "破產", "仲裁", "公寓大廈", "家庭暴力", "家暴"]
...
def run_single_task_safe(manifest: dict, manifest_path: Path, exclusive_key: str, qm: QuotaManager, chunking_only: bool) -> bool:
    ...
    key_display = exclusive_key[:8] if exclusive_key else "None"
...
def _run_task_steps(manifest: dict, manifest_path: Path, paths: dict, exclusive_key: str, chunking_only: bool) -> bool:
    ...
    try:
        ...
        # 發送核心心跳 (Heartbeat)，通知 sre_watchdog 系統仍在運行
        if heartbeat_event:
            heartbeat_event.signal()
...
if __name__ == "__main__":
    ...
    if not lock.acquire():
        print(f"[Workflow Lock] Another instance of run_workflow is already running (locked via KernelMutex). Exiting.")
        sys.exit(0)
        
    job = None
    try:
        # Phase 7.8: 建立 Job Object 並綁定當前進程，用於自動管理與統一斬殺
        try:
            job = KernelJobObject(JOB_NAME)
            job.assign_current_process()
            print("[OS Kernel] 成功建立並綁定 Windows Job Object。")
        except Exception as e:
            print(f"[OS Kernel Error] Job Object 綁定失敗: {e}")
