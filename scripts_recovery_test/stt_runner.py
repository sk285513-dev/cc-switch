...
def _call_with_timeout(func, timeout_sec, *args, **kwargs):
    ...
    try:
        ...
        print(f"[Timeout] STT API call stuck for {timeout_sec}s, forcing exit.")
        executor.shutdown(wait=False, cancel_futures=True)
        raise TimeoutError("Vertex API Deadlock Timeout in STT")
    finally:
...
def process_single_chunk(
    ...
    while chunk["retry_count"] < retry_limit:
        try:
            # ── 雲端 Gemini 路徑 (Smart Fallback Dual-Routing) ──
            if stt_engine in ["gemini", "vertexai"]:
                ...
                if stt_engine == "gemini":
                    ...
                    def _init_vertex_client():
                        ...
                        return genai.Client(vertexai=True, project=v_project, location=v_loc, credentials=creds)

                    try:
                        fresh_client = _call_with_timeout(_init_vertex_client, 10)
                    except TimeoutError:
                        logging.error(f"[STT] Vertex AI Auth Deadlock detected for chunk {chunk_file}. Falling back to gemini API.")
                        with key_lock:
                            active_key = current_key_ref[0]
                        fresh_client = genai.Client(api_key=active_key)
                        stt_engine = "gemini" # Force gemini fallback for this chunk

                try_base64 = True
                use_vertex = config.get("settings", {}).get("vertexai_project")
                uploaded_file = None
                fallback_to_gcs = False
                blob = None
                transcription = ""

                if try_base64:
                    try:
                        logging.info(f"[Parallel STT] 嘗試 Base64 InlineData 傳輸 {chunk_filename}...")
                        ...
                        is_429 = isinstance(e, google_exceptions.ResourceExhausted) or getattr(e, 'code', None) == 429 or "429" in err_str
                        is_413 = "413" in err_str or "Payload Too Large" in err_str
                        
                        if use_vertex and (is_429 or is_413):
                            logging.warning(f"[Parallel STT] Base64 遭阻斷 (429/413)！啟動 GCS 退避防禦...")
                ...
                if fallback_to_gcs:
                    ...
                    storage_client = storage.Client(project=use_vertex, credentials=credentials)
                    bucket = storage_client.bucket(f"lexmind-temp-{use_vertex}")
                    
                    if not bucket.exists():
                        bucket = storage_client.create_bucket(f"lexmind-temp-{use_vertex}", location=config.get("settings", {}).get("vertexai_location", "us-central1"))
                    
                    blob_name = f"{task_id}_{chunk_filename}"
                    blob = bucket.blob(blob_name)
                    logging.info(f"[Parallel STT] 退避至 GCS，上傳 {chunk_filename}...")
                    blob.upload_from_filename(chunk_path)
                    uploaded_file = types.Part.from_uri(file_uri=f"gs://{bucket.name}/{blob.name}", mime_type="audio/wav")
                    
                    # 必須切換為 Vertex Client 才能存取 gs:// URI
                    v_loc = config.get("settings", {}).get("vertexai_location", "us-central1")
                    vertex_client = genai.Client(vertexai=True, project=use_vertex, location=v_loc, credentials=credentials)
                    
                    try:
                        transcription = transcribe_chunk(vertex_client, uploaded_file, config)
                    finally:
                        if blob is not None and blob.exists():
                            try:
                                blob.delete()
                                logging.info(f"[Parallel STT] GCS 清理完成 (閱後即焚): {blob.name}")
                            except Exception as del_e:
                                logging.warning(f"[Parallel STT] GCS 清理失敗: {del_e}")
            ...
            from google.api_core import exceptions as google_exceptions
            if stt_engine == "gemini" and isinstance(e, (
...
def run_stt(task_id: str, exclusive_key: str = None):
    ...
    if stt_engine == "gemini":
        ...
        if os.path.exists(cred_abs):
            ...
            logging.info(f"[STT] 找到憑證檔案：{cred_abs}。將使用該 JSON 登入 Vertex AI。")
        else:
            logging.info(f"[STT] 未偵測到 {cred_abs}，將直接依賴原生 ADC 認證登入 Vertex AI。")
            
        v_project = config.get("settings", {}).get("vertexai_project")
        v_loc = config.get("settings", {}).get("vertexai_location", "us-central1")
        logging.info(f"[STT] 使用 Vertex AI 企業通道，專案：{v_project} ({v_loc})")
    ...
    if all_succeeded:
        ...
        logging.warning(f"STT Phase completed with errors (Succeeded=False). Skipping Merge Agent to prevent incomplete transcripts.")
